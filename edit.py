...
# ── Fix uvloop conflict with Gradio 6 / uvicorn ───────────────
# Must be set BEFORE server/gradio threads start
...
def edit_image(init_image_path, mask_image_path, pos_prompt, neg_prompt,
               steps, cfg, seed, denoise, unet_opt, clip_opt, vae_opt):
    ...
    print("[1/4] Encoding prompts... ", end="", flush=True)
    t0 = time.time()
    ...
    print("[2/4] Latent conditioning... ", end="", flush=True)
    t0 = time.time()
...
def process_gradio(editor_data, pos_prompt, neg_prompt, steps, cfg, seed, denoise,
    ...
    print(f"  Prompt   : {pos_prompt[:80]}{'...' if len(pos_prompt) > 80 else ''}")
    print("="*50 + "\n")
...
demo.launch(
    ...
    css=custom_css,
    allowed_paths=[f"{root_path}/results"]
)
